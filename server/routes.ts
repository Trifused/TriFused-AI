...
export async function registerRoutes(
  ...
  // Image proxy to bypass CORS restrictions for social preview cards
  app.get("/api/proxy-image", async (req, res) => {
    try {
      ...
      const response = await fetch(imageUrl, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; Vibe2A/1.0)',
          'Accept': 'image/*',
        },
      });
    ...
    try {
      ...
      if (grade.qrCodeData && grade.shareToken) {
        ...
        try {
          ...
          // Share URL next to QR code - use request host for correct domain
          const requestHost = req.get('host') || 'trifused.com';
          const protocol = req.protocol || 'https';
          const shareUrl = `${protocol}://${requestHost}/report/${grade.shareToken}`;
      ...
      const footerHost = req.get('host') || 'trifused.com';
      const footerProtocol = req.protocol || 'https';
      const footerUrl = `${footerProtocol}://${footerHost}/grader`;
      page.drawText("Generated by Website Grader", {
        x: margin,
        y: yPos - 5,
        size: 9,
        font: helvetica,
