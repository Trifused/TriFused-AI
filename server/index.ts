  ...
  try {
    ...
    if (webhookBaseUrl && webhookBaseUrl !== 'https://undefined') {
      try {
        ...
        console.log(`Webhook configured: ${result?.webhook?.url || 'created'}`);
      } catch (webhookError) {
        console.log('Webhook setup skipped (may require production domain)');
      }
    } else {
